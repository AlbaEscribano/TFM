...
class ImageSegmentationApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.version_history = {}  # Diccionario para almacenar el historial de versiones
    ...
    def load_previous_session(self):
        if os.path.exists('session.json'):
            with open('session.json', 'r') as file:
                ...
                self.deleted_images = data.get('deleted_images', [])
    ...
    def save_image(self):
        ...
        if file_path:
            versioned_file_path = self.get_versioned_file_path(file_path)
            imsave(versioned_file_path, (self.segmented_image > 0).astype(np.uint8) * 255)
            self.update_version_history(file_path, versioned_file_path)
            QMessageBox.information(self, "Success", "Segmented image saved successfully.")

    def get_versioned_file_path(self, file_path):
        base, ext = os.path.splitext(file_path)
        timestamp = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
        return f"{base}_{timestamp}{ext}"

    def update_version_history(self, original_path, versioned_path):
        if original_path not in self.version_history:
            self.version_history[original_path] = []
        self.version_history[original_path].append(versioned_path)

    def show_version_history(self, file_path):
        if file_path in self.version_history:
            versions = self.version_history[file_path]
            version_list = "\n".join(versions)
            QMessageBox.information(self, "Version History", f"Versions for {file_path}:\n{version_list}")
        else:
            QMessageBox.information(self, "Version History", "No versions available for this file.")
